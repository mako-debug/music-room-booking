...
> **注意：** 此為初版設計文件（2026-04-16）。實作過程中新增了多項功能，最新的完整功能清單請參考 [README.md](../../../README.md)，操作說明請參考 [user-guide.md](../../user-guide.md)。

### 初版之後新增的功能
- 帳號管理頁面 `/admin`（新增/修改/停用/啟用/重設密碼/改 Email）
- 管理員可代其他老師預約
- 老師色系自訂（15 色）
- 語音輸入（🎤 Web Speech API）
- 使用者自行修改姓名（同步更新課表）和密碼
- 帳號停用/啟用（Firebase Auth disabled）
- 登出/取消預約二次確認
- 週檢視今日藍色醒目標示
- 日曆表頭暖色底色 + 活潑字體標題
- Firebase Admin SDK API route（密碼重設/Email 修改/帳號停用）
